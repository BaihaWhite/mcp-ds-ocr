...
// 配置持久化：~/.dsh/dsh-ocr.json（dsh-ssh 同款文件模式，不依赖 settings 服务）；
// 工具 ocr_recognize（OpenAI / Anthropic 双协议视觉 OCR）；
// 路由 /api/dsh-ocr/config|models|test 供卡片调用。
...
// 规范化 base URL：兼容用户直接粘贴完整端点（去掉 /chat/completions、/v1/messages、/models 等后缀）
...
function basePath(cfg, suffix) {
  const base = normalizeBase(cfg.apiurl)
  if (cfg.apiFormat === 'anthropic') {
...
// ---------- apply ----------
// 配置持久化采用 dsh-ssh 同款模式（~/.dsh/dsh-ocr.json + 路由），
// 不依赖 settings 服务（用户 patch 层插件 ctx 中不可见，inject 永不触发）
...
export function apply(ctx, config) {
  const CONFIG_PATH = path.join(process.env.HOME || '/root', '.dsh', 'dsh-ocr.json')
  let store = { ...DEFAULT_CONFIG }
  let loaded = false

  // 加载/保存 ~/.dsh/dsh-ocr.json（节点完整 fs，直接可用）
  const loadStore = async () => {
    if (loaded) return
    try {
      const text = await readFile(CONFIG_PATH, 'utf8')
      const parsed = JSON.parse(text)
      if (parsed && typeof parsed === 'object') store = { ...DEFAULT_CONFIG, ...parsed }
    } catch (e) {
      console.log(`[dsh-ocr] config load skipped (${CONFIG_PATH}): ${msg(e)}`)
    }
    loaded = true
  }
  const saveStore = async () => {
    try {
      ...
      console.log(`[dsh-ocr] config save failed (${CONFIG_PATH}): ${msg(e)}`)
      return false
    }
  }
  void loadStore()
  ...
  const resolve = () => ({
    apiurl: String(store.apiurl || ''),
    apikey: String(store.apikey || ''),
    model: String(store.model || ''),
    apiFormat: store.apiFormat === 'anthropic' ? 'anthropic' : 'openai',
    threshold: typeof store.threshold === 'number' ? clamp(store.threshold, 0, 1) : 0.7,
    coordMode: ['percent', 'pixel', 'both'].includes(store.coordMode) ? store.coordMode : 'both',
    retryEnabled: store.retryEnabled !== false,
    maxRounds: Math.max(1, Math.min(3, Number(store.maxRounds) || 2)),
  })
  const masked = () => {
    ...
    return {
      apiurl: store.apiurl,
      model: store.model,
      apiFormat: store.apiFormat,
      hasKey: !!key,
      keyHint: key ? (key.length <= 6 ? '****' : `${key.slice(0, 2)}****${key.slice(-4)}`) : '',
      threshold: store.threshold,
      coordMode: store.coordMode,
      retryEnabled: store.retryEnabled,
  ...
  const mergeConfig = (patch) => {
    if (typeof patch.apiurl === 'string') {
      const v = patch.apiurl.trim().replace(/\/+$/, '')
      if (/^https?:\/\//i.test(v) && /^[A-Za-z0-9._~:\/?#\[\]@!$&()*+,;=%\-]+$/.test(v.replace(/'/g, ''))) store.apiurl = v
    }
    if (typeof patch.apikey === 'string' && patch.apikey.trim() && !patch.apikey.includes('****')) store.apikey = patch.apikey.trim()
    if (typeof patch.model === 'string' && patch.model.trim()) store.model = patch.model.trim()
    if (patch.apiFormat === 'openai' || patch.apiFormat === 'anthropic') store.apiFormat = patch.apiFormat
    if (typeof patch.threshold === 'number' && patch.threshold >= 0 && patch.threshold <= 1) store.threshold = patch.threshold
    if (patch.coordMode === 'percent' || patch.coordMode === 'pixel' || patch.coordMode === 'both') store.coordMode = patch.coordMode
    if (typeof patch.retryEnabled === 'boolean') store.retryEnabled = patch.retryEnabled
    if (typeof patch.maxRounds === 'number' && patch.maxRounds >= 1 && patch.maxRounds <= 3) store.maxRounds = Math.floor(patch.maxRounds)
  }
  ...
  // ---------- 卡片按钮路由（loopback-only + 同源校验，仿 dsh-ssh） ----------
  ...
  // 读取 POST JSON body（上限 64KB），用于表单草稿临时配置
  ...
  // 合并表单草稿配置（POST body 覆盖；未提供字段回落到已保存配置）
  const effectiveConfig = async (req) => {
    const cfg = resolve()
    if (req.method !== 'POST') return cfg
    const body = await readJsonBody(req)
    if (!body || typeof body !== 'object') return cfg
    return {
      ...cfg,
      apiurl: typeof body.apiurl === 'string' && body.apiurl.trim() ? normalizeBase(body.apiurl) : cfg.apiurl,
      apikey: typeof body.apikey === 'string' && body.apikey.trim() && !body.apikey.includes('****') ? body.apikey.trim() : cfg.apikey,
      apiFormat: body.apiFormat === 'anthropic' || body.apiFormat === 'openai' ? body.apiFormat : cfg.apiFormat,
    }
  }

  ctx.webServer.register({
    kind: 'exact',
    path: '/api/dsh-ocr/config',
    handler: async (req, res) => {
      if (!isLoopbackRequest(req)) return writeJson(res, 403, { ok: false, error: 'forbidden: loopback-only' })
      await loadStore()
      const method = req.method ?? 'GET'
      if (method === 'GET') {
        writeJson(res, 200, { ok: true, config: masked() })
        return
      }
      if (method === 'POST') {
        const body = await readJsonBody(req)
        if (!body || typeof body !== 'object') return writeJson(res, 200, { ok: false, error: 'invalid JSON body' })
        mergeConfig(body)
        const saved = await saveStore()
        if (!saved) return writeJson(res, 200, { ok: false, error: `配置写入失败：${CONFIG_PATH}` })
        writeJson(res, 200, { ok: true, config: masked(), savedToFile: true })
        return
      }
  ...
  ctx.webServer.register({
    ...
    handler: async (req, res) => {
      if (!isLoopbackRequest(req)) return writeJson(res, 403, { ok: false, error: 'forbidden: loopback-only' })
      try {
        const cfg = await effectiveConfig(req)
