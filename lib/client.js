...
`
    ...
    function OcrCard(props) {
      ...
      const saveAll = async () => {
        ...
        try {
          ...
          for (const f of FIELDS) {
            ...
            if (raw === '') {
              if (saved(key)) writes.push(scope.clear(key))
              continue
            }
            let parsed
            ...
            writes.push(scope.storeValue(key, parsed))
          }
      ...
      const fetchModels = async () => {
        ...
        setStatus('正在获取模型列表…')
        try {
          const res = await fetch('/api/dsh-ocr/models', { headers: { 'X-Requested-With': 'dsh-ocr' } })
      ...
      const testConfig = async () => {
        ...
        setStatus('正在测试连接…')
        try {
          const res = await fetch('/api/dsh-ocr/test', { headers: { 'X-Requested-With': 'dsh-ocr' } })
          const body = await res.json()
      ...
      return React.createElement('div', { className: 'dsh-ocr-card' },
        ...
        expanded ? React.createElement('div', { className: 'dsh-ocr-body', key: 'body' }, [
          ...
          React.createElement('div', { className: 'dsh-ocr-row', key: 'r2' }, [
            ...
            React.createElement('input', { key: 'i', id: 'ocr-apikey', type: 'password', placeholder: saved('apikey') ? '已设置，留空则不修改' : 'sk-…', value: display('apikey'), onChange: edit('apikey'), spellCheck: false, autoComplete: 'off' }),
          ]),
          // Model Name + 旁侧下拉
