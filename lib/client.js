...
`
    ...
    function OcrCard(props) {
      ...
      const saveAll = async () => {
        ...
        setStatus('保存中…')
        try {
          // 直接调 settings.mutate RPC（SettingsScope 会静默吞错，这里显示真实原因）
          if (!api || !api.settings || !api.settings.mutate) {
            setStatus('保存失败：settings RPC 不可用（connection 未注入）')
            return
          }
          const ops = []
          for (const f of FIELDS) {
            ...
            if (raw === '') {
              ...
              if (saved(key)) ops.push({ op: 'unset', path: [key] })
              continue
            }
            let parsed
          ...
          if (ops.length === 0) { setStatus('没有改动'); return }
          const res = await api.settings.mutate({ ns: NAMESPACE, ops })
          if (!res || !res.result || !res.result.ok) {
            const detail = res && res.result && res.result.error ? (res.result.error.message || JSON.stringify(res.result.error)) : '未知错误'
    ...
    function apply(ctx) {
      ...
      ctx.slots.inject('settings.plugin.item', function* () {
        yield ctx.slots.register({
          ...
          order: 95,
          label: () => 'OCR 视觉识别',
        }, (props) => React.createElement(OcrCard, { ...props, scope, api }))
      })
    }
